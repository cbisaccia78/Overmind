...
def test_dispatch_all_branches(tmp_path: Path):
    ...
    shell = gateway._dispatch(
        "run_shell",
        {"command": "echo hi", "timeout_s": 9, "allow_write": True, "write_subdir": "tmp"},
    )
    ...
    write_big = gateway._dispatch("write_file", {"path": "out.txt", "content": "y" * 20})
    assert write_big["ok"] is False
    assert write_big["error"]["code"] == "size_limit"

    # write_file success
    write_ok = gateway._dispatch("write_file", {"path": "dir/out.txt", "content": "abc"})
    ...
    store_ok = gateway._dispatch("store_memory", {"collection": "docs", "text": "abc", "metadata": {}})
    assert store_ok == {"ok": True, "item": {"id": "m1"}}

    gateway.memory.search.return_value = [{"id": "m1", "score": 0.9}]
    search_ok = gateway._dispatch("search_memory", {"collection": "docs", "query": "a", "top_k": 1})
